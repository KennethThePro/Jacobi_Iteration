...
def jacobi(A, b, x_init, epsilon, max_iterations):
    ...
    for i in range(max_iterations):
        ...
        print("New x Value After " + str(i+1) + " Iterations\n")
        print(x_new)
        print("\nThe Norm of Matrix After Finding Matrix of xNew - xOld is", np.linalg.norm(x_new - x))
        print("\n====================================================================================")
        ...
        if np.linalg.norm(x_new - x) < epsilon:
            print("The Jacobi Method Succeeded After " + str(i+1) + " Iterations")
            return x_new
        #else continue to iteration
        x = x_new
    if (i+1 == max_iterations) and np.linalg.norm(x_new - x) > epsilon:
        print("The Jacobi Method Has Failed After " + str(max_iterations) + " Iterations")
    return x
...
# get TOL
episilon = eval(input("Enter TOL: "))
dp = str(episilon)[::-1].find('.')
...
#call jacobi method
x = jacobi(A, b, x_init, episilon, max_iterations)
